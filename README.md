...
This fork uses psc-package instead of bower. If you haven't already installed it, do so with:
...
``` shell
git clone https://github.com/bjarkevad/purescript-halogen-template.git my-halogen-project
...
Finally you'll need to install the PureScript library dependencies for this project with psc-package:
...
This will start a watch process that uses [parcel](https://github.com/parcel-bundler/parcel) to rebundle the app whenever the _output_ files are changed. Since `purs ide` rebuilds modules on save, this means you can use this much faster bundle-only rebuild script.
